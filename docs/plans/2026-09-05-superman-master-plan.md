...
工作台必须同时支持**桌面 Web + 手机 H5**（不是原生 App，是响应式 H5 + PWA）：

- **断点**：mobile <768px / desktop ≥768px；所有新视图移动优先（mobile-first）写样式
- **移动端导航**：底部 tab bar（阅读 / 审批台 / 热点 / 我的），桌面端维持侧边栏；同一组件树按断点切换壳层
- **触控**：可点目标 ≥44px；审批卡片支持手势——右滑准奏、左滑驳回（与 J/K/A/R 快捷键平级，桌面显示快捷键提示、移动显示手势提示）
- **PWA**：manifest + service worker，可「添加到主屏幕」；提词器页支持全屏 + 屏幕常亮（wake lock）
- **提词器移动优先**：口播场景主要在手机上录制，该页以竖屏手机为第一设计目标
- **性能**：移动端首屏只载当前视图 bundle，路由级 code splitting；图片懒加载
- 验收：Chrome DevTools 375px/768px/1440px 三档截图走查，核心流（看队列→审批→看热点）在 375px 下无横向滚动、无遮挡
...
| M1d | 移动端 H5 自适应（响应式 + PWA + 手势，见 §4.0） | 2 天 |
| M2 | 洗稿三平台草稿 | 2-3 天 |
| M3 | 口播稿 + 提词器 | 2 天 |
...
### 内核迁移与产品完善（2026-09-05 用户拍板插入，优先级最高）

| 批次 | 内容 | 状态 |
|---|---|---|
| K1-K4 | DSH 内核迁移（骨架/业务/界面/AI 指挥台） | ✅ 已完成 |
| **P2a** | 消息中心（进底部菜单第五 tab） | 进行中 |
| **P2b** | 治理 v2：归一化 + 跨平台合并 + 关键词方向分类 | 待做 |
| **P2c** | AI 方向分类（选题/搞钱/学习）+ 方向配额 | 待做 |
| **P2d** | 发布后表现追踪（快照抓取 + 火了提示反哺选题） | 待做 |
| **P2e** | 平台授权中心（公众号→抖音→小红书） | 待做 |
| K5 | 退役 Next.js + 部署上线 | 功能齐后再做 |

设计细节见 `2026-09-05-governance-v2-design.md`、平台授权见 `reports/superman-publish-auth-research.md`（工作区 reports/）。
